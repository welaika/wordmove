...
describe Wordmove::RemoteHost do
  ...
  it "should not use ports if missing" do
    ...
    @host.should_not_receive(:locally_run).with("rsync", "-azLK", "-e",  "ssh -p 30000", anything(), anything(), "--delete", "username@host:foobar/", "barfoo" ) 
    @host.upload_dir("foobar", "barfoo" )
  end
  ...
  it "should skip password files when not using passwords" do
    ...
    @host.should_not_receive(:locally_run).with("rsync", "-azLK", /--password-file.*/, anything(), "--delete", "username@host:foobar/", "barfoo" ) 
    @host.upload_dir("foobar", "barfoo" )
  end
